...
private val userRepo: UserRepository) : ViewModel() {
    ...
    var searchResult :MutableStateFlow<List<UiPost>> = MutableStateFlow(listOf())
    ...
    fun searchPosts(text: String) {
        viewModelScope.launch (Dispatchers.IO){
            ...
            postRepo.searchPostsByTitle(text).collect{entityList ->
                val newList = entityList.map{ UiPost(pfpURL, "Mohammed Edrees",
                    it.publishedAt, it.title, it.body, it.upvotes, 1500) }
